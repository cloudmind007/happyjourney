...
const Sidebar: React.FC<SidebarProps> = ({ collapsed, setCollapsed }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      // Assuming mobile screen width is less than 768px (standard Tailwind 'md' breakpoint)
      if (window.innerWidth < 768) {
        setCollapsed(true);
      } else {
        setCollapsed(false); // Optionally expand on larger screens
      }
    };
  ...
  return (
    <div
      className={`h-full ${
        collapsed ? "w-20" : "w-64"
      } bg-white shadow-sm border-r transition-all duration-300 md:w-64`} // Ensure sidebar is expanded on medium screens and above
    >
      <div className="flex items-center justify-between p-4">
        {!collapsed && (
          <h1 className="text-xl font-bold text-gray-800">RELSWAD</h1>
